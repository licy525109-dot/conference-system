...
function createPrismaMock() {
  ...
  const matches = (item: typeof items[number], where: { userId?: string; id?: string; readAt?: null }) =>
  ...
  const mock = {
    ...
    userNotification: {
      findMany: async (args: { where: { userId: string; readAt?: null }; skip: number; take: number }) => items
        .filter((item) => matches(item, args.where))
        .sort((left, right) => right.createdAt.getTime() - left.createdAt.getTime())
        .slice(args.skip, args.skip + args.take),
      count: async (args: { where: { userId: string; readAt?: null } }) => items.filter((item) => matches(item, args.where)).length,
      findFirst: async (args: { where: { id: string; userId: string } }) => items.find((item) => matches(item, args.where)) ?? null,
      update: async (args: { where: { id: string }; data: { readAt: Date } }) => {
      ...
      updateMany: async (args: { where: { userId: string; readAt: null }; data: { readAt: Date } }) => {
