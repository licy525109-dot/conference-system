<template>
  <section class="identity-panel">
    ...
    <template v-if="data">
      ...
      <div v-for="attendee in data.attendees" :key="attendee.id" class="attendee-row">
        ...
        <div v-if="canEdit" class="actions">
          ...
          <el-button @click="openEditor(attendee, 'invite')">生成领取口令</el-button>
    ...
    <el-dialog v-model="claimVisible" title="参会人领取口令" width="min(600px, 94vw)">
      <p>有效期 24 小时，仅限本人验证相同手机号后领取。请私下发送给对应参会人。</p>
      <el-input :model-value="claimToken" readonly />
      <p>小程序「个人中心 → 领取参会资格」中粘贴此口令。</p>
    </el-dialog>
  </section>
</template>
...
const titles: Record<string, string> = { bind: "关联实际参会账号", unbind: "解除参会账号关联", replace: "更换实际参会人", invite: "生成参会领取口令", owner: "调整业务联系人" };
...
async function save() {
  ...
  try {
    ...
    if (result.claimToken) { claimToken.value = result.claimToken; claimVisible.value = true; }
