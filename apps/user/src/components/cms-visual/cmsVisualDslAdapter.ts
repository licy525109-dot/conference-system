...
import { isCmsComponentUserRenderable } from "@/utils/cmsComponents";
...
export function hasLegacyCmsVisualNodes(dsl: PageDsl): boolean {
  const nodes = dsl.dsl?.nodes?.filter((node) => node.enabled !== false) ?? [];
  return nodes.length > 0 && nodes.every((node) => {
    const originalType = readText(node.meta?.originalType);
    return Boolean(originalType && isCmsComponentUserRenderable(originalType));
  });
}

export function isFixedBusinessTemplateComponent(component: CmsComponent): boolean {
  if (component.type === "fixed-business-template") return true;
  return hasFixedTemplateMarker(component.config);
}
