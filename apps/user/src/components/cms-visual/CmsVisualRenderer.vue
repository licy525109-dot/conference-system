  ...
  <view :class="rootClass" :style="rootStyle">
    ...
    >
      ...
      <CmsConferenceScheduleRenderer
        v-else-if="component.type === 'conference-schedule'"
        :component="component"
        :conferences="conferences"
        :now-timestamp="nowTimestamp"
        @action="handleConferenceAction($event, component)"
        @calendar="addConferenceToCalendar"
      />
      ...
      <CmsLoginCardRenderer
        v-else-if="component.type === 'login-card'"
        :component="component"
        :user-context="effectiveUserContext"
        @activate="handleLoginCard(component)"
      />

      <CmsEntryModuleRenderer
        v-else-if="component.type === 'quick-icon-grid' || component.type === 'service-shortcut-card'"
        :component="component"
        :entries="homeEntries(component)"
        @activate="handleEntryModuleAction(component, $event)"
        @more="handleMoreAction(component)"
      />
      ...
      <view
        ...
        :class="['cms-notice', `is-${stringConfig(component, 'displayStyle') || 'elegant'}`]"
        :style="noticeStyle(component)"
      >
        <text v-if="booleanConfig(component, 'showIcon', true)" class="cms-notice__icon">{{ stringConfig(component, "iconText") || "讯" }}</text>
        <text class="cms-notice__text">{{ stringConfig(component, "text") || "报名开放中" }}</text>
      </view>
      ...
      <view v-else-if="component.type === 'my-order-list'" class="cms-section cms-orders">
        ...
        <view v-if="!isLoggedIn" class="cms-empty">登录后查看你的报名订单和商城订单。</view>
      </view>

      <CmsMallProductGridRenderer
...
import CmsConferenceScheduleRenderer from "@/components/cms-visual/component-renderers/CmsConferenceScheduleRenderer.vue";
import CmsEntryModuleRenderer from "@/components/cms-visual/component-renderers/CmsEntryModuleRenderer.vue";
import CmsHeroBannerRenderer from "@/components/cms-visual/component-renderers/CmsHeroBannerRenderer.vue";
import CmsLoginCardRenderer from "@/components/cms-visual/component-renderers/CmsLoginCardRenderer.vue";
...
const visibleComponents = computed(() =>
  props.components
    ...
    .flatMap(expandFixedBusinessTemplate)
...
const effectiveUserContext = computed<Record<string, unknown> | null>(() => {
  ...
  return {
    ...
    nickname: storedUser.value.wechatNickname || storedUser.value.nickname || "微信用户",
    avatarUrl: storedUser.value.wechatAvatarUrl || "",
    phone: storedUser.value.phone || "",
    memberStatus: "普通用户",
    memberLevel: "普通用户",
    registrationCount: 0,
...
async function handleEntryModuleAction(component: CmsComponent, entry: CmsEntryItem): Promise<void> {
  await handleEntryAction(component, entry as unknown as HomeEntryItem);
}
...
function radiusPresetRpx(component: CmsComponent): number {
  const preset = stringConfig(component, "radiusPreset") || "md";
...
function noticeStyle(component: CmsComponent): Record<string, string> {
  return {
    ...textStyle(component),
    textAlign: stringConfig(component, "textAlign") || "left"
  };
}
...
function expandFixedBusinessTemplate(component: CmsComponent): CmsComponent[] {
  if (normalizedComponentType(component) !== "fixed-business-template") return [component];
