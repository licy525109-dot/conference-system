...
const visibleComponents = computed(() =>
  runtimeComponents.value
    .filter((item) => item.enabled)
    .filter((item) => !(props.suppressRegistrationCta && isCmsRegistrationCta(item.type)))
    .sort((a, b) => a.sortOrder - b.sortOrder)
);
const runtimeComponents = computed(() => componentsFromDsl(props.dsl));
...
function componentsFromDsl(dsl: PageDsl): CmsComponent[] {
...
watch(() => runtimeComponents.value, loadCustomFonts, { deep: true });
watch(() => runtimeComponents.value, () => void loadMallProductComponents(), { deep: true });
watch(
...
function loadCustomFonts() {
  for (const component of runtimeComponents.value) {
