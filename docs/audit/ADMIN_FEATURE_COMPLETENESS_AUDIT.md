...
| 页面装修 | `/materials` | 素材管理 | `/admin/materials*` | Material, MaterialCategory, Product, ProductImage | 页面装修/商城商品 | 无 | 是 | 素材库支持改名、改分类、URL 维护、图片/视频/文件预览和软删除；默认列表隐藏已禁用素材，删除操作写审计日志且不破坏历史已发布页面。 |
...
- AI API Key 仍只来自服务器环境变量，后台只返回 `configured / masked`。
- 通知、短信、微信订阅配置页不返回供应商 Secret。
...
- 微信订阅消息和短信真实发送，需要模板、供应商账号、签名和频控规则。
- 企业微信客户群同步、权限检测、回调验签，需要企业微信自建应用权限和回调配置。
- AI 真实 provider，需要 `AI_PROVIDER`、`AI_API_KEY`、模型和费用/限流策略。
- 小程序正式版发布，需要微信平台上传、体验版验证和版本审核。

仍保留 badge 的原因：

- 会议管理中的库存预警、签到核销、签到记录、签到统计保留 `灰度`：接口和后台操作已存在，但扫码端 E2E、现场设备流程和大客流演练仍需上线前人工验证。
- 订单交易中的支付异常保留 `辅助`：该入口只做人工核查和备注，不提供自动修复支付状态，避免绕过支付回调。
- 通知中心中的微信订阅消息、短信配置保留 `辅助`：真实发送依赖外部模板和供应商配置，未配置时必须记录 `SKIPPED`。
- AI 配置保留 `辅助`：密钥由服务器环境变量管理，后台只展示配置状态，不保存明文。
