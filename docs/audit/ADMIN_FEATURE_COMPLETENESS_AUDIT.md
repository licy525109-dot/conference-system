...
| 企微客户群 | `/wecom/tasks` | WecomPage | `/admin/wecom/group-message-tasks*`, `/admin/wecom/group-message-tasks/test-send` | CustomerGroupMessageTask, CustomerGroupMessageLog, WecomCustomerGroup | 不需要 | 无 | 是 | 支持全部客户群、指定客户群、指定会议关联群和指定群测试发送；保存企业微信 msgId / taskId / 原始错误码，不做后台强发。 |
